...
class StudentGradesGrid(Grid):
    ...
    def api(self, id=None):
        ...
        for r in result_rows:
            if is_admin() and r["grade"]["submission_id_gcs"] is not None:
                row_notebook = SPAN(
                    A(I(_class="fa fa-file"), _target="_blank", _href=COLAB_BASE + r["grade"]["drive_id"]),
                    " ",
                    A(I(_class="fa fa-eye"), _target="_blank", _href=URL(
                        'admin_share', 'submission', r["grade"]["submission_id_gcs"],
                        vars=dict(title="{} by {} on {}, Submission".format(
                            r["assignment"]["name"],
                            r["grade"]["student"],
                            r["grade"]["grade_date"].isoformat(),
                        )),
                        signer=url_signer
                    )),
                    " ",
                ...
                row_notebook = A(I(_class="fa fa-file"), _target="_blank", _href=COLAB_BASE + r["grade"]["drive_id"]).xml()
            rows.append(dict(
            cells=[
